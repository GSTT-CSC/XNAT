    ...
    with xnat.connect(
        ...
        for subject in tqdm(project.subjects.values()):
            ...
            for experiment in subject.experiments.values():
                ...
                # custom code to skip already processed lunit data
                lunit_exists = False
                for scan in experiment.scans.values():
                    if "9999999" in scan.id:
                        logging.info(
                            f"Skipping {experiment.id} - lunit data already avaialble"
                        )
                        lunit_exists = True
                if lunit_exists:
                    break
                ...
                for scan in experiment.scans.values():
                    ...
                    for attempt in range(
                        ...
                        if "attempts" in config["settings"]
                        else 1
                    ):
                        try:
                            if not check_criteria(
                                scan, config["criteria"]["inclusion"]["scan"]
                            ):
                                break
                            ...
                            last_e = e
                            logging.warning(
                                f"FAILED after {attempt} attempts: {subject}, {experiment}, {scan}; exception: {last_e}"
                            )
                            log_failures(
                                f"{subject},{experiment},{scan},{last_e}", config
                            )
                            break
                        ...
                        # all attempts failed
                        logging.warning(
                            f"FAILED after {config['settings']['attempts']} attempts: {subject}, {experiment}, {scan}; exception: {last_e}"
                        )
...
def check_criteria(xnat_obj, criteria) -> bool:
    ...
    try:
        ...
        raise InclusionException(
            f"There was an error assessing this file against inclusion criteria {e}"
        )
