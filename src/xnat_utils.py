"""
XNAT REST API utils, which assumes are run inside container.
...
class XNATUtils:
    ...
    def dicom_dump(self, project, session_id, subject_id='', scan_id='', tags=''):
        ...
        url = "{}/data/services/dicomdump?src=/archive/projects/{}/experiments/{}".format(self.xnat_host, project,
                                                                                          session_id)
        if len(subject_id) > 0:
            url = "{}/data/services/dicomdump?src=/archive/projects/{}/subjects/{}/experiments/{}".format(
                self.xnat_host, project, subject_id, session_id)
        if len(scan_id) > 0:
    ...
    def upload_roi(self, project, session_id, assessor_label, dicom_path):
        ...
        url = "{}/xapi/roi/projects/{}/sessions/{}/collections/{}?type=RTSTRUCT&overwrite=true".format(self.xnat_host,
                                                                                                       project,
                                                                                                       session_id,
                                                                                                       assessor_label)
        print(url)
    ...
    def trigger_autorun(self, project, session_id):
        url = "{}/data/archive/projects/{}/experiments/{}?triggerPipelines=true".format(self.xnat_host, project,
                                                                                        session_id)
    ...
    def get_scan(self, experiment_id, scan_id, project="NO PROJECT", xsitype='ALL SCANS', format='json'):
        ...
        url = "{}/data/archive/projects/{}/experiments/{}/scans/{}?format={}".format(self.xnat_host, project,
                                                                                     experiment_id, scan_id, format)
        if "NO PROJECT" in project:
            url = "{}/data/archive/experiments/{}/scans/HALJa{}?".format(self.xnat_host, experiment_id, scan_id)
    ...
    def get_assessor(self, assessor_id, experiment_id, project="NO PROJECT", format='json'):
        # any experiment type - assessor included. id can be label IF porject specified
        url = "{}/data/archive/projects/{}/experiments/{}?format={}".format(self.xnat_host, project, assessor_id,
                                                                            format)
        if "NO PROJECT" in project:
            url = "{}/data/archive/experiments/{}/assessors/{}?format={}".format(self.xnat_host, experiment_id,
                                                                                 assessor_id, format)
        print(url)
        r = self.session.get(url)
        json_data = r.text
    ...
    def get_assessors(self, experiment_id, project="NO PROJECT", subject="NONE", format='json'):
        url = "{}/data/archive/projects/{}/experiments/{}/assessors?format={}".format(self.xnat_host, project,
    ...
    def get_experiment(self, experiment_id, project="NO PROJECT", format='json'):
        # any experiment type - assessor included. id can be label IF porject specified
        url = "{}/data/archive/projects/{}/experiments/{}?format={}".format(self.xnat_host, project, experiment_id,
                                                                            format)
        if "NO PROJECT" in project:
    ...
    def share_session(self, project, session_label, project_to_share_to):
        url = "{}/data/archive/projects/{}/experiments/{}/projects/{}".format(self.xnat_host, project, session_label,
                                                                              project_to_share_to)
        r = self.session.put(url)
