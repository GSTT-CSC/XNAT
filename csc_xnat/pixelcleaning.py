...
# This modifies the original file, but is ok as this script is 
# intended to run on copied datasets, never on original data in the XNAT archive.
...
deid_path = os.path.join(script_dir, "deid.custom")
...
# run cleaning
            result = clientcustom.detect(target)
            print(result)
...
            if target != fullpath:
                clean_target = os.path.join(clean_folder, os.path.basename(target))
                clean_final = os.path.join(clean_folder, os.path.basename(fullpath))
                os.rename(clean_target, clean_final)
