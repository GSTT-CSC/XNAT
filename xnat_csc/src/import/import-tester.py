...
for studies in batch:
    ...
    for study in studies['studies']:
        ...
        try:    
            ...
            relabelmap = "{{  \"Subject\": \"{}\", \"Session\": \"{}\"   }}".format(subject_mapping[study['accessionNumber']], session_mapping[study['accessionNumber']])
            search_list.append('\"relabelMap\":   {}  '.format(relabelmap))

            search_query = ",".join(search_list)
     

            searchterm = '{{ \"aeTitle\": \"{}\",  \"forceImport\": true,  \"pacsId\": {},  \"port\": {},  \"projectId\": \"{}\",  \"studies\": [  {{     {}   }} ] }}'.format(aet, pacs_id, port, project,  search_query)
